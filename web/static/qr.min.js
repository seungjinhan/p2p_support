/**
 * Lightweight Standalone QRCode Generator for Browser (Pure JavaScript)
 * Generates SVG/Canvas QR codes client-side with zero backend dependencies.
 */
(function(root) {
    ...
    QRCodeModel.prototype = {
        addData: function(data) {
            ...
            this.dataList.push(new QR8bitByte(utf8));
            this.dataCache = null;
        },
    ...
    /**
     * Render QR Code to HTML element as SVG or Canvas
     */
    function QRCode(element, text, size) {
        size = size || 180;
        var qr = new QRCodeModel(0, 0); // Low error correction for maximum QR density
        qr.addData(text);
        qr.make();
        ...
        for (var row = 0; row < moduleCount; row++) {
            for (var col = 0; col < moduleCount; col++) {
                if (qr.isDark(row, col)) {
                    ...
                    rect.setAttribute('x', (col * cellSize).toFixed(2));
                    rect.setAttribute('y', (row * cellSize).toFixed(2));
                    rect.setAttribute('width', (cellSize + 0.5).toFixed(2));
                    rect.setAttribute('height', (cellSize + 0.5).toFixed(2));
